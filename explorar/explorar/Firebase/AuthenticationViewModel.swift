...
class AuthenticationViewModel: ObservableObject {
    ...
    @Published var shouldShowMissingUsernameAlert: Bool = false
    ...
    private func wait() async {
        do {
            ...
            Drops.show(Drop(title: "ErrorX \(error.localizedDescription)"))
        }
    }
...
extension AuthenticationViewModel {
    ...
    func signUpWithEmailPassword(userName: String) async -> Bool {
        authenticationState = .authenticating
        do  {
        ...
        if case .failure(let failure) = result {
            ...
            if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
                ...
                guard let appleIDToken = appleIDCredential.identityToken else {
                    ...
                    Drops.show(Drop(title: "Unable to fetdch identify token."))
                    return
                }
                guard let idTokenString = String(data: appleIDToken, encoding: .utf8) else {
                    print("Unable to serialise token string from data: \(appleIDToken.debugDescription)")
                    Drops.show(Drop(title: "Unable to serialise token string from data: \(appleIDToken.debugDescription)"))
                    return
                }
                ...
                Task {
                    do {
                        ...
                        FirestoreService.shared.checkIfUserExists { result in
                            ...
                            case .success(let exists):
                                if exists {
                                    ...
                                    TelemetryDeck.signal("Logged_In_With_Apple")
                                    completion(false)
                                } else {
                                    print("User NOT exist, create")
                                    TelemetryDeck.signal("Registered_With_Apple")
                                    completion(true)
                                }
                        ...
                        Drops.show(Drop(title: "Error authenticating: \(error.localizedDescription)"))
                        completion(false)
                    }
                }
            }
                ...
                do {
                    ...
                    case .authorized:
                        ...
                        FirestoreService.shared.checkIfUserExists { result in
                            ...
                            case .success(let exists):
                                if !exists {
                                    print("User does NOT exist-> show alert")
                                    self.shouldShowMissingUsernameAlert = true
                                } else {
