                    ...
                    if pointOfInterest.images.count == imageCount {
                        withAnimation {
                            let items = images.enumerated().map { (idx, img) in
                                DetailImage(id: "\(idx)", title: "", image: img, previewImage: img)
                            }
                    ...
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        VStack {
                            if !imagesAreDownloading && images.count > 0 {
                                HStack {
                                    Spacer()
                                    CardStackView(images: images, selectedCardIndex: $selectedCardIndex, imageSize: CGSize(width: 220, height: 300), coordinator: coordinator)
