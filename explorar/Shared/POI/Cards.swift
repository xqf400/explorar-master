...
struct CardStackView: View {
    ...
    init(images: [UIImage], selectedCardIndex: Binding<Int>? = nil, imageSize: CGSize = CGSize(width: 100, height: 150), coordinator: UICoordinator) {
        var createdItems = [Card]()
        for(id, image) in images.enumerated() {
            createdItems.append (Card(id: id, image: image, zIndex: Double (images.count - 1 - id)))
        }
